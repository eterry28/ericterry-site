...
const ComradeColaMachine = () => {
  ...
  const [messages, setMessages] = useState([
    "Comrade Cola Machine initialized with 5 colas!",
  ]);
  ...
  const insertCredit = () => {
    switch (state) {
      ...
      case STATES.EMPTY:
        addMessage(
          "❌ You can't insert credits because this machine is out of Comrade Cola."
        );
        break;
      case STATES.SOLD:
        addMessage(
          "⏳ Please wait. Your cold, refreshing Comrade Cola is being dispensed."
        );
        break;
      default:
        addMessage(
          "⚠️ Whoa there comrade! We need to raise some sort of error."
        );
    }
  };
  ...
  const returnCredit = () => {
    switch (state) {
      ...
      case STATES.EMPTY:
        addMessage(
          "❌ There are no credits to return because sold out machines don't accept credits."
        );
        setState(STATES.NO_CREDIT);
        break;
      case STATES.SOLD:
        addMessage(
          "❌ Sorry Comrade. You already bought a cold, refreshing Comrade Cola!"
        );
        setState(STATES.NO_CREDIT);
        break;
      default:
  ...
  const customerChoice = () => {
    switch (state) {
      ...
      case STATES.NO_CREDIT:
        addMessage(
          "❌ You've made a choice, but haven't inserted any credits."
        );
        break;
      case STATES.EMPTY:
        addMessage(
          "❌ You've made a choice, but we are sold out of Comrade Cola."
        );
        break;
      case STATES.SOLD:
        addMessage(
          "❌ Sorry Comrade. You only get one cold, refreshing Comrade Cola per credit!"
        );
        break;
      default:
        addMessage(
  ...
  const dispense = () => {
    ...
    setTimeout(() => {
      switch (state) {
        case STATES.SOLD:
          addMessage(
            "🥤 Congrats Comrade! Enjoy your cold, refreshing Comrade Cola!"
          );
  ...
  const refill = () => {
    if (inventory > 0) {
      addMessage("❌ Machine still has inventory. No need to refill yet.");
      return;
    }
    setInventory(5);
    setState(STATES.NO_CREDIT);
            ...
            >
              <button
                ...
                style={{
                  ...
                  borderRadius: "8px",
                  cursor:
                    animating || state === STATES.HAS_CREDIT
                      ? "not-allowed"
                      : "pointer",
                  transition: "all 0.3s",
                  textTransform: "uppercase",
